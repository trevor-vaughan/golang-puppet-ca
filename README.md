...
- **Subject alternative name policy:** `allow_subject_alt_names` (default `false`, matching OpenVox Server) decides whether a submitted CSR may request Subject Alternative Names of its own. With it off, a CSR asking for any SAN beyond a DNS entry equal to its own certname is refused at signing time rather than trimmed — TLS peers match on the SAN set, so an unconstrained request is a request to be somebody else, including this CA's own server name. A renewal keeps whatever SANs its certificate already holds, and the admin-only generate paths (`openvox-ca generate --dns` and `POST /generate/{subject}?dns=`) are unaffected, since their names come from an administrator rather than from an agent's CSR. See [subject alternative names requested by a CSR](docs/configuration.md#subject-alternative-names-requested-by-a-csr)
